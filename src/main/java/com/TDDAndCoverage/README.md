...
۲. در فرایند ایجاد نرم‌افزار هم تیم ایجاد (Development Team) و هم تیم تضمین کیفیت (QA Team) وظیفه تست نرم‌افزار را برعهده دارند. هر کدام از این تیم‌ها بیش‌تر با کدام دسته از انواع تست سروکار دارد؟ چرا؟ انواع تست ذکر شده را به طور مختصر شرح دهید.
  ...
  پاسخ: <br><br>تیم توسعه:<br>
  تیم توسعه عمدتاً در مراحل اولیه تست و تست‌هایی که مربوط به کد منبع اصلی هستند نقش دارد. تست‌هایی که این تیم معمولاً انجام می‌دهد عبارتند از:<br><br>

&emsp;۱. تست واحد (Unit Testing) : این نوع تست کمترین سطح تست نرم‌افزار است که در آن اجزای کوچک و واحدهای کد به طور جداگانه تست می‌شوند.<br>
هدف این تست اطمینان از صحت عملکرد هر واحد کد به صورت مجزا از دیگر قسمت‌ها است.<br>
دلیل استفاده از این نوع تست این است که توسعه‌دهندگان معمولاً قطعه های کوچک کد را تست می‌کنند و مستقیماً می‌توانند مشکلات را حل کنند.<br><br>

&emsp;۲.تست یکپارچه‌سازی (Integration Testing): این نوع تست به بررسی عملکرد ترکیبی از چندین واحد کد که با هم کار می‌کنند می‌پردازد.<br>
هدف این تست اطمینان از آن است که وقتی اجزای مختلف کد با هم ادغام می‌شوند، به درستی کار می‌کنند.<br>
دلیل استفاده از این تست این است که توسعه‌دهندگان نیاز دارند بدانند که ماژول‌ها و کامپوننت‌های مختلف کد به درستی با هم تعامل می‌کنند.<br><br>

&emsp;۳.تست Smoke Testing: این نوع تست اولیه و سطح بالا از نرم‌افزار است که پس از آنکه نرم‌افزار جدید یا به روزرسانی شد، اجرا می‌شود.<br>
هدف آن تضمین این است که نسخه ارائه شده به کمترین مقدار مناسب کار کند.<br>
دلیل استفاده از این تست این است که توسعه‌دهندگان می‌خواهند اطمینان حاصل کنند که قسمت‌های عمده نرم‌افزار قبل از ارسال به تیم QA بدون مشکل کار می‌کنند.<br><br>

تیم تضمین کیفیت:<br>
تیم تضمین کیفیت معمولاً در مراحل بعدی تست و تست‌هایی که نیاز به ارزیابی جامع‌تری دارند، نقش دارد. تست‌هایی که این تیم بیشتر با آن‌ها سروکار دارد شامل موارد زیر است:<br><br>

&emsp;۱.تست عملکرد (Functional Testing): این نوع تست بررسی می‌کند که آیا نرم‌افزار تمامی نیازمندی‌های عملکردی که از آن انتظار می‌رود را انجام می‌دهد یا خیر.<br>
دلیل استفاده از این تست این است که تیم QA باید اطمینان حاصل کند که نرم‌افزار از دیدگاه کاربر نهایی، تمام ویژگی‌ها و عملکردهای مورد نیاز را فراهم می‌کند.<br><br>

&emsp;۲.تست رگرسیون (Regression Testing): این تست برای اطمینان از آن است که تغییرات جدید یا رفع اشکالات موجود در نرم‌افزار باعث ایجاد مشکلات جدیدی نشده باشند.<br>
دلیل استفاده از این تست این است که تیم QA باید اطمینان حاصل کند که پس از افزودن ویژگی‌ها یا رفع اشکالات، نرم‌افزار به درستی کار می‌کند و مشکلات جدیدی ایجاد نمی‌شود.<br><br>

&emsp;۳.تست سیستم (System Testing): این تست به بررسی کامل نرم‌افزار به عنوان یک کل از دیدگاه پایان کاربر می‌پردازد.<br>
هدف این تست اطمینان از عملکرد کلی نرم‌افزار در محیط واقعی است.<br>
هدف از این تست این است که تیم QA باید اطمینان حاصل کند که کل سیستم در شرایط واقعی به درستی کار می‌کند.<br><br>

&emsp;۴.تست بار و فشار (Load & Stress Testing): این نوع تست به بررسی عملکرد نرم‌افزار در شرایط بار سنگین و شرایط استرس‌زا می‌پردازد.<br>
هدف از این تست این است که تیم QA باید اطمینان حاصل کند که نرم‌افزار تحت بار زیاد (تعداد بالای کاربران) یا شرایط غیرعادی (مثلاً پردازش داده‌های بسیار زیاد) به درستی کار می‌کند.<br><br>
...
- تست `testStudentHasBookInReturningBook`: در این تست، یک کتاب و یک دانش آموز را به کتابخانه اضافه می‌کنیم و بدون اینکه کتاب را به دانش آموز قرض بدهیم، تلاش می‌کنیم تا آن‌را با استفاده از تابع returnBook به کتابخانه برگردانیم. در این حالت، این تابع باید false برگرداند و خطوط مربوط به بررسی شرط اینکه دانش آموز کتاب را دارد، پوشش داده شود. در این حالت مشاهده می‌کنیم که میزان پوشش خطوط کلاس Library، حدود ۴ درصد دیگر نیز افزایش می‌یابد:

![Screenshot from 2024-07-28 21-48-11](https://github.com/user-attachments/assets/490523bb-d4ea-49ca-8bea-21ef09b2cca9)

ما توانستیم که میزان پوشش خطوط این کلاس را از عدد ۷۲ درصد، با اضافه کردن تست‌های معناداری که از روی گزارش قبلی استخراج شده‌اند، به حدود ۸۶ درصد برسانیم.

گزارش مربوط به پوشش، پس از اضافه شدن این تست‌ها، در آدرس src/main/resources/coverage-reports/Q4 قرار دارند. در تصویر زیر، تفاوت دو گزارش پوشش مرحله‌ی ۳ و ۴ نمایش داده شده‌اند:

![پوشش پیش از اضافه شدن تست‌ها](https://github.com/user-attachments/assets/ba1d6951-f900-4cfd-8673-bd73c6fecde5)

![پوشش پس از اضافه شدن تست‌ها](https://github.com/user-attachments/assets/c91601be-85f1-4d31-bfdb-79af579dc082)
