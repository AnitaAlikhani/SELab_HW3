...
۱. تست واحد (Unit Testing) : این نوع تست کمترین سطح تست نرم‌افزار است که در آن اجزای کوچک و واحدهای کد به طور جداگانه تست می‌شوند.<br>
...
۲.تست یکپارچه‌سازی (Integration Testing): این نوع تست به بررسی عملکرد ترکیبی از چندین واحد کد که با هم کار می‌کنند می‌پردازد.<br>
...
۳.تست Smoke Testing: این نوع تست اولیه و سطح بالا از نرم‌افزار است که پس از آنکه نرم‌افزار جدید یا به روزرسانی شد، اجرا می‌شود.<br>
...
۱.تست عملکرد (Functional Testing): این نوع تست بررسی می‌کند که آیا نرم‌افزار تمامی نیازمندی‌های عملکردی که از آن انتظار می‌رود را انجام می‌دهد یا خیر.<br>
...
۲.تست رگرسیون (Regression Testing): این تست برای اطمینان از آن است که تغییرات جدید یا رفع اشکالات موجود در نرم‌افزار باعث ایجاد مشکلات جدیدی نشده باشند.<br>
...
۳.تست سیستم (System Testing): این تست به بررسی کامل نرم‌افزار به عنوان یک کل از دیدگاه پایان کاربر می‌پردازد.<br>
...
۴.تست بار و فشار (Load & Stress Testing): این نوع تست به بررسی عملکرد نرم‌افزار در شرایط بار سنگین و شرایط استرس‌زا می‌پردازد.<br>
